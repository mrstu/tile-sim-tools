...
{
  char inpath[150], prefix[150], outfile[200], outprefix[200], metafile[150], gridfile[100], timespan[10];
  FILE **infilehandle;
  ...
  // MRS: added so that exit status would be provided
  return 0;
} // END PROGRAM
...
struct GridInfo *Calc_Grid(char *inpath, char *prefix)
{
  ...
  while((filep = readdir(dp)) != NULL) {
    if (!strncmp(filep->d_name,prefix,strlen(prefix))) {
      ...
      k++;
        ...
        if (!strncmp(filep->d_name,"no_data",7)) {
  ...
  if (NCELLS > 1) {
      ...
      if (first_time) {
        ...
        if (fabs(lats[k] - temp) >= 0.00001 && delta_lat > fabs(lats[k] - temp)) {
          delta_lat = fabs(lats[k] - temp);
        	...
        	// address case where only one row or one column of valid cells
        	delta_lat = 0.0625;
        }
        temp = lats[k];
      }
    ...
    // Calculate NROWS, NCOLS
	MINROW = 1;
	MAXROW = (maxlat - minlat)/0.0625 + 1;
	NROWS = MAXROW;
	MINCOL = 1;
	MAXCOL = (maxlon - minlon)/0.0625 + 1;
	NCOLS = MAXCOL;
	delta_lat = 0.0625; //1;
	delta_lon = 0.0625; //1;
